...
**Wannier band-match — CONFIRMED (2026-07-04, upgrades the auto-proxy gate).** Overlaid the 6-band d+s Wannier interpolation (EPW `band_plot` from `epwdata.fmt`) against DFT bands (`pw.x` bands + `bands.x`) on an identical 201-point Γ–X–W–L–Γ–K path. Method: `filkf` path + `band_plot=.true. epwread=.true. elph=.true. a2f/phonselfen=.false.` (elph must be true or `ephwann_shuffle`/`plot_band` is skipped; and `filkf` requires the fine-grid `nkf*/nqf*` lines removed). **Result: EXACT at Γ** (Wannier ≡ DFT to sub-meV: 12.441/19.137×3/21.391×2 eV), 5/6 sub-meV at X; **median deviation ≈25–30 meV at the Fermi surface** (E_F±0.5 eV), with outliers up to ~1 eV at isolated k (where the smooth 6-D disentangled subspace cuts across DFT band-character changes). Wannier spreads are clean: 6 atom-centred WFs, spreads 1.07–2.49 Ų (the 2.49 is the diffuse s), Ω_D=0.007, Ω_OD=0.55 Ų. **Verdict: the d+s Wannier functions faithfully represent the manifold — the λ=1.10 is not a mis-Wannierization artifact — but the match is decent-not-pristine (a production λ would tighten windows to pull the ~1 eV outliers below ~10 meV).** Plot: sent to operator; raw artifacts in scratch (`band.eig`, `ir_dft.bands.gnu`).
