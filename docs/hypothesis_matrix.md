...
| H7-singlet | Under a **singlet** pairing assumption, a static moment pair-breaks the state: the critical field is suppressed and Pauli-limited (Chandrasekhar-Clogston, Bc_pauli = 1.86·Tc) | `magnetic_field.py` (`pairing_critical_field`, `PairingSymmetry.SINGLET`) | `field_response_ratio_admissible` | The measured critical field exceeds the Pauli limit in the **clean limit** (`field_response_ratio_admissible` > 1 — the clean-limit-gated R_Pauli, admissible only when Maki α ≥ 1.8 and clean; only a triplet can host that, and a dirty/unknown candidate cannot register it) |
