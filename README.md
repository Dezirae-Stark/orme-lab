...
- **Ledger tab.** The falsify-first Hudson Claim Ledger (HC-01…HC-08) as a live dashboard — a claim × material matrix, per-material and integrated (max-lineage, min-claim-over-core) roll-up cards, and evidence-control panels you can drive by hand. Every gate reads default-blocked until you supply real evidence; the strongest attainable state is "Independently Replicated" and the string "HUDSON CLAIM VALIDATED" is never rendered. Same status ladder and thresholds as [`hudson_ledger.py`](src/orme_lab/hudson_ledger.py), parity-tested against it. A per-material two-branch flow visualizes conventional superconductivity (`G_conventional_superconductivity`) and the Hudson optical mechanism (`G_hudson_mechanism`) as independent, animated tracks — the two branches never merge, and a material can clear one without the other. Atop the tab, a guarded 3D "material-state stage" (`web/ledger3d.js`) renders the focused material as a schematic cluster icon with a six-gate ring and an O_H polariton — reading the same parity-locked gates as the matrix, never a new claim — plus a lineage breadcrumb for click-to-focus; it degrades to a text-only fallback note and legend when WebGL is unavailable, and the 2D dashboard is fully usable either way.
